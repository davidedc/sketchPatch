...
(function () {
  ...
  initializeProcessingjs = function () {
    
	if (programAsFetched === '' ) {
	setTimeout("initializeProcessingjs()", 500);
	return;
	}

	// if, once the program is loaded, it's still the case that we can't show
	// the canvas (cause flash is running), then we just return never to try
	// to initialize processing.js again
	if (!doShowHTML5ProcessingCanvas) return;
	
	var canvas = document.getElementsByTagName('canvas');

	//alert("found these many canvases: " + canvas.length + " and the program is: " + programAsFetched);
    

    for (var i = 0, l = canvas.length; i < l; i++) {
        Processing(canvas[i], programAsFetched);
