...
throw new SyntaxError('JSON.parse');};}}());"use strict";
...
//alert('get elements by tag named script: ' + root.getElementsByTagName('script'));
//do{
//alert('removing child at '+ i + ' ' + scripts[i]);
//root.removeChild(scripts[i]);i-=1;
//}
//while(i>=0);
...
try{fun(id,dom,adsafe_lib,root[1]);}
catch(ignore){}}}
try{f(dom,adsafe_lib);}catch(e){return error();}
root=null;adsafe_lib=null;},id:function(id){if(adsafe_id){return error();}
adsafe_id=id;adsafe_lib={};},isArray:Array.isArray||function(value){return Object.prototype.toString.apply(value)==='[object Array]';},later:function(func,timeout){if(typeof func==='function'){setTimeout(func,timeout||0);}else{return error();}},lib:function(name,f){if(!adsafe_id){return error();}
...
return error();},_intercept:function(f){interceptors.push(f);}};}());"use strict";ADSAFE._intercept(function(id,dom,lib,bunch){lib.cookie={get:function(){var c=' '+document.cookie+';',s=c.indexOf((' '+id+'=')),v;try{if(s>=0){s+=id.length+2;v=JSON.parse(c.slice(s,c.indexOf(';',s)));}}catch(ignore){}
