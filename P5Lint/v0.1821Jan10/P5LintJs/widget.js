...
function trim(str, chars) {
    ...
    function P5Report (thePassedProgram, originalProcessingProgram) {
            ...
            if (data.implieds) {
				...
				if (!usedExtendsOrImplements){
					...
					for (i = 0; i < data.implieds.length; i += 1) {
						...
						problemsSummariesArray.push('Undeclared function/variable: ' + theImpliedData);
            ...
            if (data.unused) {
                ...
                for (i = 0; i < data.unused.length; i += 1) {
					...
					if (thePassedProgram.match(isItReallyUnused).length == 1){
						...
						problemsLinesArray.push(data.unused[i].line);
						problemsSummariesArray.push('Warning: unused function/variable: ' + data.unused[i]['function']);
						problemsSnippetsArray.push('');
...
function P5LintDoTheChecks(input,output) {
        
        //function P5LintChecks() {
		//alert('starting P5LintChecks function');
// Make a JSON cookie of the option object.
			...
			var strippedProgram = addLineNumbers(input,document.getElementById("WITH_LINE_NUMBERS"));
			//alert('theClassList');
            ...
            var theReport = P5Report(finalSourceCode,input);
			
			var fullDebugReport = '';

			///////////////////////////////
			if (!document.getElementsByName("tolerate_text_fonts_checkbox")[0].checked){
			...
			///////////////////////////////
			if (!document.getElementsByName("tolerate_print_println_checkbox")[0].checked){
			...
			if (!document.getElementsByName("tolerate_3d_checkbox")[0].checked){
				//alert('checking 3d uses');
				var regexForFinding3dUse = new RegExp("size\\s*\\(\\s*[0123456789]*\\s*,\\s*[0123456789]*\\s*,\\s*(P3D|OPENGL)\\s*\\)\\s*;",'g');
			...
			/////////////////////////////////
			if (!document.getElementsByName("tolerate_imports_checkbox")[0].checked && someImportsHaveBeenFound){
